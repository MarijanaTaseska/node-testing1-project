...
describe('[Exercise 3] findLargestInteger', () => {
  test('[5] returns the largest number in an array of objects { integer: 2 }', () => {
    ...
    const input1 = [{ integer: 5 }, { integer: 3 }, { integer: 2 }]
    const input2 = [{ integer: 1 }, { integer: 3 }, { integer: 9 }]
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  ...
  test('[21] rejects an error with the message:"number must be a number" if passed a non number type ', async () => {
    try {
      await utils.isEvenNumberAsync('fooobar')
    } catch (error) {
      expect(error.message).toMatch(/number must be a number/i)
    }
  })
  test('[22] rejects an error with the message:"number must be a number" if passes NaN', async () => {
    try {
